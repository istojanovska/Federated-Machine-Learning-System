...
public class ModelWeightsService {
    private final String pythonUrl = "http://localhost:8002/train-local-model";
    ...
    @RabbitListener(queues = "aggregated_weights_diabetes_queue")
    public void receiveDiabetesModel(AggregatedModel model) {
        recieveAggregatedWeights(model);
    }

    @RabbitListener(queues = "aggregated_weights_anemia_queue")
    public void receiveAnemiaModel(AggregatedModel model) {
        recieveAggregatedWeights(model);
    }

    @RabbitListener(queues = "aggregated_weights_heart_attack_queue")
    ...
    public void sendTrainedModelToFlServer(ModelWeights trainedModel) {
        ...
        String queueName = switch (trainedModel.getModelTypeId()){
            case 0 -> "received_weights_diabetes_queue";
            case 1 -> "received_weights_anemia_queue";
            case 2 -> "received_weights_heart_attack_queue";
